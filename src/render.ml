...
let init_scene (particle_system : Particle_system.t) (camera : Camera.t) : scene
  ...
  and mushrooms =
    let get_mushroom (fungi_lists : node list list) : node list list =
      List.map (fun lst -> ([List.nth lst (Random.int (List.length lst))])) fungi_lists
    in
      Nodes (List.map (fun n -> Nodes n)
      (get_mushroom
      (load_objs_to_objects default_program
        "assets/mushrooms.obj"
        [Mat4.(translate ~x:0.0 ~y:0.0 ~z:0.0 * scale_uniform 1.0);
        Mat4.(translate ~x:(rand 5.0 1.0) ~y:0.0 ~z:(rand 2.5 1.0) * scale_uniform 1.0);
        Mat4.(translate ~x:(rand 2.9 1.0) ~y:0.0 ~z:(rand 2.4 1.0) * scale_uniform 1.0);
        Mat4.(translate ~x:(rand 2.9 1.0) ~y:0.0 ~z:(rand 3.5 1.0) * scale_uniform 1.0);
        Mat4.(translate ~x:(rand 2.7 1.0) ~y:0.0 ~z:(rand 2.5 1.0) * scale_uniform 1.0)])))
  and trees =
    ...
    let rec place_trees n (lst : (Mat4.t list)) = (match n with
      0 -> lst
      | _ -> (place_trees (n - 1) ((Mat4.(translate ~x:(rand 4000.0 400.0) 
                                                  ~y:(-50.0) 
                                                  ~z:(rand 4000.0 400.0) * 
